---
layout: post
title: Analisis de textos
author: Soledad Retamar
published: true
status: process
draft: false
tags: consejo
---

```{r setup} knitr::opts_chunk$set(echo = FALSE)
```

## Actas de reuniones del Consejo Superior de UTN
En este post trataremos de reproducir el trabajo realizado por .

En este caso hemos tomado lactas de reuniones del Consejo Superior de los √∫ltimos 10 a√±os.
Los paquetes necesarios ser√°n: NLP, openNLP y magrittr

```{r, echo=FALSE}

library(pdftools)
library(rvest)
library(readr)
library(purrr)
library(stringr)
library(stringi)
library(dplyr)
library(tidyr)
library(tidyverse)
library(tidytext)
library(tibble)
library(tau)
library(tokenizers)
library(rcorpora)
library(viridis)
library(tm)
library(wordcloud)
library(ggplot2)

actas_CS <-   read.csv("data\\actas.csv", sep="\t", stringsAsFactors=FALSE, encoding="latin1" )
names(actas_CS) <- c("archivo","anio", "nro", "reunion", "lugar", "txt")

```

### Cantidad de Reuniones por AÒo
En primer lugar analizaremos cuantas actas por aÒo se recuperaron, que serÌa equivalente a la cantidad de reuniones llevadas a cabo

```{r, echo=FALSE}
reuniones <- actas_CS[actas_CS$anio != "2005" & actas_CS$anio != "3.pd" & actas_CS$anio != "AU20" & !is.na(actas_CS$anio),] %>% 
            count(anio, reunion, sort = TRUE) %>% 
            group_by(anio)
anios <- as.data.frame(reuniones$anio, reuniones$re)
names(anios)<-"anio"

Freuniones<- ggplot(anios, aes(x=as.factor(anios$anio), 
    fill = as.numeric(anios$anio))) + 
  geom_bar(position='dodge', show.legend = FALSE) + 
  xlab("Anio") + 
  ylab("Cantidad") + 
  scale_x_discrete("AÒos" ) + 
  theme_minimal() +
  ggtitle("Reuniones de CS UTN")
Freuniones

```
